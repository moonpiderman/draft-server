...
import com.wafflestudio.draft.dto.CourtDTO
...
class CourtApiController(private val courtService: CourtService) {
    @GetMapping("/")
    fun getCourtsV1(@Valid @ModelAttribute request: CourtDTO.GetCourtsRequest): ListResponse<CourtDTO.CourtResponse> {
        val courts = courtService.findCourtsByName(request.name.orEmpty())
        return ListResponse(courts.map { CourtDTO.CourtResponse(it) })
    }
}
