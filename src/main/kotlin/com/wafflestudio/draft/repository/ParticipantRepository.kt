...
interface ParticipantRepository : JpaRepository<Participant?, Long?> {
    ...
    @Query("SELECT new com.wafflestudio.draft.dto.response.UserInformationResponse(u.id, u.username, u.email, u.profileImage) " +
            // "SELECT new com.wafflestudio.draft.dto.reponse.UserInformationResponse(u.id, u.username, u.email, u.profileImage) " +
            "FROM Participant p " +
            "INNER JOIN p.user u " +
            "WHERE p.room = :room " +
            "AND p.team = :team")
    fun getUsersInTeam(
            ...
            @Param("team") team: Team): List<UserDTO.UserInformationResponse>
